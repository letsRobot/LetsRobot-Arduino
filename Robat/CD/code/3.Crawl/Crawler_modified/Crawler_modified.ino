...
//put your code here ---------------------------------------------------------
	...
	while (1)
	{
		if (radio.available())
		{
//      if (radio.read(&order, 1)) //changed
      if (radio.available()) // from this line       
			{
        radio.read(&order, 1); // to this line
				if (order > 0)
				{
					Serial.print("Order:");
					Serial.print(order);
...
//      while (radio.read(&order, 1)); //changed
      while (radio.available()) { // from this line 
        radio.read(&order,1); // to this line
      }
		}
